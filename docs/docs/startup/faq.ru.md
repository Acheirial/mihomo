...
> * `go124`/`go125`: скомпилирована с использованием Golang 1.24 / 1.25. Эта версия предназначена для совместимости с определенными операционными системами или архитектурами. Без тега `go` код компилируется с использованием последней версии Golang (форк MetaCubeX 1.26). Минимальный компилятор — Go 1.24.
...
👉[Узнайте больше](https://go.dev/doc/go1.24#ports) о системной совместимости версии Golang 1.24
...
Для пользователей Linux: Согласно [go wiki](https://go.dev/doc/go1.24#linux), Go 1.24 поддерживает только ядра версии 3.2 и выше. Официальные сборки теперь требуют минимум Go 1.24; теги `go123`/`go122`/`go120` больше не выпускаются.
