...
Транспортный уровень, поддерживает `tcp`/`ws`/`http`/`h2`/`grpc`/`xhttp`/`mkcp`/`mekya`. `httpupgrade` рассматривается как `ws` плюс HTTP Upgrade. Если не настроен или настроен с другим значением, используется tcp
