...
Управляет интенсивностью гонки адресов одной семьи. Значение по умолчанию `false` **больше не ждёт таймаут первого IP**. Соединения используют чередование в стиле RFC 8305 (задержка 300 мс): мёртвая первая запись A/AAAA переключается за время delay, а не блокирует на `DefaultTCPTimeout` (5 с).

`true` запускает адреса одной семьи с delay=0 (старая одновременная гонка SYN). Dual-stack по-прежнему сразу стартует основную семью, а запасную — только после delay; запасная семья не должна слать SYN в t=0.

Соединения TFO всегда последовательны и наследуют контекст вызывающего.

Семью можно зафиксировать через `ip-version` узла. Не используйте `tcp-concurrent` как выключатель Happy Eyeballs — HE это путь по умолчанию.
