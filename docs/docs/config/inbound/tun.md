...
可用值： `system/gvisor/mixed/mips/go`
...
!!! note "协议栈之间的区别"
    ...
    * `go` 是 Linux 上的纯 Go 协议栈，DIRECT TCP/UDP 可走内核 splice；本窗口默认仍是 gvisor，需手动 `stack: go`。ICMP echo 由栈本地回，不走 PrepareConnection 直连 ping。
    * [简单性能测试](tun.md#tun_1)
