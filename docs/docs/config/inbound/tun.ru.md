...
Возможные значения: `system/gvisor/mixed/mips/go`
...
!!! note "Различия между стеками протоколов"
    ...
    * `go` — чистый стек на Go; в Linux DIRECT TCP/UDP может использовать splice. По умолчанию по-прежнему `gvisor`; для включения задайте `stack: go`. ICMP echo отвечает локально, без пересылки через PrepareConnection.
