...
4. **`dns→tunnel` 常量提升**:`DnsRespectRules` 从 `tunnel/dns_dialer.go` 提到 `constant/dns.go`,`dns/dialer.go` 的 `RespectRules` 别名改指 `constant`。**`dns` 对 `tunnel` 的 import 保留**——`dns` 还依赖 `tunnel.DNSDialer`/`NewDNSDialer`,那是历史结构(上游亦如此),搬常量解决不了,留白。
...
- **`dns→tunnel` 完全解耦**(最强论据:component→tunnel/statistic 与 dns→tunnel.DNSDialer 是同类倒挂,一起治更彻底):否决,`DNSDialer` 的依赖注入改造影响面广且上游同构,收益不抵风险,留白给后续批次。
...
- 留白:`dns→tunnel.DNSDialer` 倒挂、`tunnel` listener 的 `ReCreate` 复制粘贴、`hub/route` 业务下沉、`statistic` 依赖注入。
